...
__global__ void fill_ghost(cell_t *cur, int ext_n)
{
    ...
    cur[LEFT_GHOST] = cur[RIGHT_GHOST - HALO];
    cur[RIGHT_GHOST] = cur[LEFT_GHOST + HALO];
}
...
__global__ void cuda_step(cell_t *cur, cell_t *next, int ext_n)
{
    ...
    if (gindex < ext_n - 1)
    {
        ...
        if (threadIdx.x == 0)
        {
            const int len = (BLKDIM < ext_n - 1 - gindex ? BLKDIM : ext_n - 1 - gindex);
            buf[0] = cur[gindex - 1];
            buf[len + 1] = cur[gindex + len];
        }
...
int main(int argc, char *argv[])
{
    ...
    // const int RIGHT = RIGHT_GHOST - HALO;
    ...
    const int NBLOCKS = (width + BLKDIM - 1) / BLKDIM;
    ...
    for (int s = 0; s < steps; s++)
    {
        ...
        cuda_step<<<NBLOCKS, BLKDIM>>>(d_cur, d_next, ext_width);
