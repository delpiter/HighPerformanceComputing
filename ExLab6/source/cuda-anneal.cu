...
#define BLKDIM2D 32
#define BLKDIM (BLKDIM2D * BLKDIM2D)
...
__global__ void copy_top_bottom(cell_t *grid, int ext_width, int ext_height)
{
    ...
    if (gindex < ext_width)
    {
        *IDX(grid, ext_width, BOTTOM_GHOST, gindex) = *IDX(grid, ext_width, TOP, gindex); /* top to bottom halo */
        *IDX(grid, ext_width, TOP_GHOST, gindex) = *IDX(grid, ext_width, BOTTOM, gindex); /* bottom to top halo */
    }
...
__global__ void copy_left_right(cell_t *grid, int ext_width, int ext_height)
{
    ...
    int gindex = threadIdx.x + blockIdx.x * blockDim.x;
    if (gindex < ext_height)
    {
        *IDX(grid, ext_width, gindex, RIGHT_GHOST) = *IDX(grid, ext_width, gindex, LEFT); /* left column to right halo */
        *IDX(grid, ext_width, gindex, LEFT_GHOST) = *IDX(grid, ext_width, gindex, RIGHT); /* right column to left halo */
    }
...
int main(int argc, char *argv[])
{
    ...
    /* 1D blocks to initialize the ghost area */
