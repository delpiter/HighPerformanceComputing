...
__global__ void reverse_kernel(int *in, int *out, int n)
{
    ...
    if (index < n)
    {
        ...
        out[n - index - 1] = in[index];
    }
}

__global__ void reverse_kernel_inplace(int *array, int n)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    if (index < n / 2)
    {
        int mirror = n - index - 1;
        int tmp = array[index];
        array[index] = array[mirror];
        array[mirror] = tmp;
    }
}
...
void inplace_reverse(int *in, int n)
{
    int *d_reverse;
    const int SIZE = n * sizeof(int);
    cudaSafeCall(cudaMalloc((void **)&d_reverse, SIZE));

    cudaSafeCall(cudaMemcpy(d_reverse, in, SIZE, cudaMemcpyHostToDevice));

    reverse_kernel_inplace<<<((n / 2) + BLKDIM - 1) / BLKDIM, BLKDIM>>>(d_reverse, n);
    cudaCheckError();

    cudaSafeCall(cudaMemcpy(in, d_reverse, SIZE, cudaMemcpyDeviceToHost));
