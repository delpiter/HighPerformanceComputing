...
float dot(const float *x, const float *y, int n)
{
    ...
    /* If it fails sets a global variable with the error code IT DOES NOT STOP THE EXECUTION*/
    cudaCheckError(); /* Introduces an Explicit synchronization barrier */
    // float result = 0.0;
    // for (int i = 0; i < n; i++) {
    //     result += x[i] * y[i];
    // }
