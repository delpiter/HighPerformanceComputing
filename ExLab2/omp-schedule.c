...
{
    /* Parallelize the following loop, simulating a
...
#pragma omp parallel default(none) shared(vin, vout, n, chunk_size)
    {
        ...
        int STRIDE = num_threads * chunk_size;
...
void do_dynamic(const int *vin, int *vout, int n)
{
    ...
    */
    const int chunk_size = 1; /* can be set to any value >= 1 */
    int current_index = 0;
#pragma omp parallel default(none) shared(current_index, chunk_size, vin, vout, n)
    {
        const int num_threads = omp_get_num_threads();
        int STRIDE = num_threads * chunk_size;
        int local_start;
...
#pragma omp flush(current_index)
