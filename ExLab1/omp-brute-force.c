...
int main(int argc, char *argv[])
{
    ...
    int found = 0;           /* Solution possible: volatile int found = 0 */

    const double initial = omp_get_wtime();
// ‘KEY_LEN’ not specified in enclosing ‘parallel’ -> Variable which visibility was not specified.
// private(k) Can also use c99 and put the k variable declaration inside the loop
#pragma omp parallel default(none) shared(KEY_LEN, msglen, n, found, enc, CHECK_LEN, check) private(k)
    {
...
#pragma omp flush(found)
    ...
    const double elapsed = omp_get_wtime() - initial;
