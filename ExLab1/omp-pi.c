...
unsigned int generate_points_unoptimal(unsigned int n)
{
    unsigned int n_inside = 0;

    int n_threads = omp_get_max_threads();
    int *thread_in = (int *)calloc(n_threads, sizeof(int));

#pragma omp parallel default(none) shared(n, thread_in) /* Non Optimized Version */
    {
        const int my_id = omp_get_thread_num();
        unsigned int my_seed = 17 + 19 * my_id;
